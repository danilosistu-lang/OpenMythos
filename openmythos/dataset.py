...
# Well-known text-column names, tried in order before falling back to the
# first string-typed parquet column.  Keeps arbitrary corpora (e.g.
# Ultra-FineWeb-EDU's ``content``) working without per-dataset config.
_TEXT_COLUMN_PREFERENCES = (
    "text", "content", "txt", "body", "document", "doc", "data", "story",
)


def _pick_text_column(schema) -> str:
    """Return the parquet column that holds raw text (schema-driven).

    Resolution order: a well-known column name (:data:`_TEXT_COLUMN_PREFERENCES`),
    then the first string/large_string column in file order.  Raises a clear
    error when the file has no string column at all instead of failing later
    with a confusing pyarrow field lookup.
    """
    by_lower = {n.lower(): n for n in schema.names}
    for pref in _TEXT_COLUMN_PREFERENCES:
        if pref in by_lower:
    ...
    for field in schema:
        if str(field.type) in ("string", "large_string"):
            return field.name
    raise RuntimeError(
        "no usable text column in parquet schema "
        f"{list(schema.names)} -- expected one of "
        f"{_TEXT_COLUMN_PREFERENCES} or any string column"
    )
...
class FineWebStreamDataset(torch_data.IterableDataset):
    ...
    def _native_doc_iter(self, epoch: int) -> Iterator[Dict[str, Any]]:
        ...
        if self.local_corpus_dir:
            ...
            if owners > len(plan):
                # More (rank x worker) readers than shards: file-modulo
                # sharding would starve every reader beyond len(plan) -- the
                # rotation loop then spins on an empty file list, logging
                # hundreds of "epoch done" lines per second.  Instead every
                # reader shares one shard deterministically and stays
                # disjoint via row-batch striding below.
                owned = [plan[owner % len(plan)]]
                shared_plan = True
                if not getattr(self, "_shared_plan_warned", False):
                    logger.warning(
                        "[dataset][native] %d parquet shard(s) but %d readers "
                        "(world_size x num_workers); sharing shards with "
                        "row-batch striding so every reader stays fed.",
                        len(plan), owners,
                    )
                    self._shared_plan_warned = True
            else:
        ...
        for rel in owned:
            ...
            with pq.ParquetFile(local_path) as pf:
                col = _pick_text_column(pf.schema_arrow)
                batch_idx = 0
                for batch in pf.iter_batches(batch_size=512, columns=[col]):
                    take = True
                    if shared_plan:
                        # Disjoint slice of the shared file for this reader:
                        # batch order is deterministic per file, so a modulo
                        # stride partitions rows without duplication.
                        take = (batch_idx % owners) == (owner % owners)
                    batch_idx += 1
                    if not take:
                        continue
        ...
        while True:
            try:
                if self.data_mode == "native":
                    ...
                    open_failures = 0
                    docs_this_pass = 0
                    for doc in self._native_doc_iter(epoch):
                        docs_this_pass += 1
                        yield doc
                    ...
                    if docs_this_pass == 0:
                        # An empty pass would otherwise rotate in a tight,
                        # CPU-spinning loop.  Say it once, loudly, and pace.
                        logger.warning(
                            "[dataset][native] shard window produced ZERO "
                            "documents (epoch %d); check the corpus schema "
                            "or --max_parquet_shards. Rotating anyway.",
                            self.meta.epochs_rotated,
                        )
                        time.sleep(1.0)
                    elif (self.meta.epochs_rotated == 1
                          or self.meta.epochs_rotated % 25 == 0):
                        logger.info(
                            "[dataset][native] shard window consumed; "
                            "rotating order (epoch %d done). Raise "
                            "--max_parquet_shards for more unique data.",
                            self.meta.epochs_rotated,
                        )
                    continue
