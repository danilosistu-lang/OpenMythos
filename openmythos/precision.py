...
SUPPORTED_PRECISIONS = ("bf16", "fp16", "fp8", "fp4", "fp32")
...
def get_autocast_context(precision: str, device_index: int = 0):
    ...
    if key == "fp16":
        if not torch.cuda.is_available():
            logger.info("No CUDA device visible; fp16 autocast disabled (CPU run).")
            return NullPrecisionContext()
        if _device_capability(device_index) and _device_capability(device_index)[0] >= 8:
            logger.warning(
                "fp16 requested on Ampere+ silicon: bf16 matches fp16 GEMM "
                "throughput with a wider mantissa - prefer --precision bf16."
            )
        logger.info(
            "FP16 autocast active (pair with the trainer's auto GradScaler; "
            "fp16 tensor cores are the pre-Ampere workhorse)."
        )
