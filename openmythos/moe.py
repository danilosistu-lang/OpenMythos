...
class MythosMoE(nn.Module):
    ...
    def __init__(
        ...
        #: Populated after every forward; live on the autograd graph.
    ...
    def _route(self, x: torch.Tensor
        ...
        Returns:
            ...
            z_loss    scalar tensor: squared log-partition penalty on the
                      router logits (ST-MoE "z-loss").  Damps logit
                      magnitude drift, which stops top-k assignments from
                      flip-flopping between steps -- a major source of
        ...
        with torch.autocast(device_type=x.device.type, enabled=False):
            ...
            # z-loss on the raw router logits (fp32, autocast-free):
            # penalises log-partition magnitude => bounded gate sharpness.
            z = torch.logsumexp(logits, dim=-1).pow(2).mean()

        self.last_router_entropy = float(-(probs.log() * probs).sum(-1).mean().detach())
        return top_p.to(x.dtype), top_i, aux, z
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        weights, idx, aux, z = self._route(flat)
